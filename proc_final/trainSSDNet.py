...
data_dir=Basic.DATADIR+'/mn/Data/weiyumei/dataset/data/Tfrecords'
model_dir='logs'
...
def ssd_model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope(params['model_scope'], default_name=None, values=[features], reuse=True):
...
def main():
    ...
    # import tensorflow
    # import tensorflow.contrib.estimator.python.estimator

    # \tensorflow\contrib\estimator\python\estimator
    # replicate_ssd_model_fn =  tf.estimator.python.estimator.replicate_model_fn(ssd_model_fn, loss_reduction=tf.losses.Reduction.MEAN)

    ssd_detector = tf.estimator.Estimator(
        model_fn=ssd_model_fn, model_dir=model_dir, config=run_config,
        params={
            'num_gpus': num_gpus,
